...
/**
 * Adapter for Local Filesystem dataset candidates.
 */
public class LocalSourceAdapter implements TrainingDataSourceAdapter {

    @Override
    public boolean supports(DataSourceCandidate candidate) {
        return candidate != null && ("LOCAL".equalsIgnoreCase(candidate.getProvider()) || "file".equalsIgnoreCase(candidate.getSourceType()));
    }
    ...
    public DatasetSource createSource(DataSourceCandidate candidate, TrainingDataAcquisitionRequest request) {
        ...
        return new LocalDatasetSource(candidate.getUri().replace("file:", ""));
    }
}
