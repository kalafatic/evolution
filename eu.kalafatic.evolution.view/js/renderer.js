...
window.ChatApp.Renderer = {
    renderMessage: function(m) {
        ...
        const safeSender = window.ChatApp.Utils.escapeHtml(m.sender || '');
        header.innerHTML = `<span class="timestamp">${m.timestamp || ''}</span><span class="icon">${icon}</span><span class="sender">${safeSender}</span>`;
        ...
        if (isDarwin) {
            ...
            } catch(e) { console.error("FORMAT_TEXT ERROR:", e); }
        } else if (!isUser) {
             try {
    ...
    formatText: function(text, role) {
        ...
        console.log("CLEAN TEXT:", clean, "INCLUDES BRACE:", clean.includes('{'));
    ...
    renderDarwin: function(m) {
        ...
        try {
            ...
            variants.forEach((v, index) => {
                ...
                if (strategy) {
                    const safeStrategy = window.ChatApp.Utils.escapeHtml(strategy);
                    headerHtml += `<div class="branch-strategy">${safeStrategy}</div>`;
                }
