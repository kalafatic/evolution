...
public class DemoTools {
    ...
    public static class EchoToolExecutor implements ToolRegistry.ToolExecutor {
        @Override
        public Object execute(JsonNode args) {
            return Collections.singletonMap("text", args.get("text").asText());
        }
    }

    public static class TimeToolExecutor implements ToolRegistry.ToolExecutor {
        @Override
        public Object execute(JsonNode args) {
            return Collections.singletonMap("currentTime", new Date().toString());
        }
    }

    public static class SystemInfoToolExecutor implements ToolRegistry.ToolExecutor {
        @Override
        public Object execute(JsonNode args) {
    ...
    public static class CalculatorToolExecutor implements ToolRegistry.ToolExecutor {
        @Override
    ...
    public static class ProjectInfoToolExecutor implements ToolRegistry.ToolExecutor {
        @Override
        public Object execute(JsonNode args) {
            Map<String, Object> info = new HashMap<>();
    ...
    public static void registerAll(ToolRegistry registry) {
        ...
        registry.register(new Tool("echo", "Echoes back the input text", echoSchema), new EchoToolExecutor());

        // Time tool
        registry.register(new Tool("time", "Returns the current system time", new HashMap<>()), new TimeToolExecutor());

        // System info tool
        registry.register(new Tool("systemInfo", "Returns system information", new HashMap<>()), new SystemInfoToolExecutor());
        ...
        registry.register(new Tool("calculator", "Performs basic arithmetic operations", calcSchema), new CalculatorToolExecutor());

        // Project info tool
        registry.register(new Tool("projectInfo", "Returns information about the current MCP server", new HashMap<>()), new ProjectInfoToolExecutor());
    }
}
