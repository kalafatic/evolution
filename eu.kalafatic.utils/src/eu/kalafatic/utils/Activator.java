...
//		final String message = MessageFormat.format(LISTENING, "Application", "to command line arguments", argsPort);
...
//					}
//
//					ToolTip tip = new ToolTip(new Shell(Display.getCurrent()), SWT.BALLOON | SWT.ICON_INFORMATION);
		...
		if (pluginProperties == null) {
			try {
				pluginProperties = new PropertyResourceBundle(
						FileLocator.openStream(getDefault().getBundle(), new Path(PLUGIN_PROPERTIES), false));
			} catch (IOException e) {
				e.printStackTrace();
			}
		}
