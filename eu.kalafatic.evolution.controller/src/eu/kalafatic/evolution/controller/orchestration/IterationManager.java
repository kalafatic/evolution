...
public class IterationManager {
	...
	public IDarwinEngine getDarwinEngine() {
		return darwinEngine;
	}
	...
	public IterationManager(TaskContext context, SessionContainer sessionContainer, AiService aiService,
			GitManager gitManager, TaskPlanner taskPlanner, TaskExecutor taskExecutor, Evaluator evaluator,
			IDarwinEngine darwinEngine, IterationMemoryService memoryService) {
	...
	public void transition(SystemState to, TaskContext ctx) {
		...
		bus.publish(new RuntimeEvent(RuntimeEventType.SUPERVISOR_STATUS_CHANGED, ctx.getSessionId(), "Kernel",
				to.toString()).withMetadata("execId", ctx.getDeterministicExecutionId())
	...
	public String handleVariantSelection(TaskContext context, List<BranchVariant> variants, String goal)
		...
		while (true) {
			...
			// ATTACH DARWIN BRANCHES JSON: This ensures branches are rendered as part of
			// the blocking turn
			...
			if ("Reject all".equalsIgnoreCase(trimmed) || trimmed.isEmpty()) {
				context.log(
						"[KERNEL] User rejected all proposals or entered empty prompt. Regenerating within current dimension.");
				return "REGENERATE_SAME_DIMENSION";
			}

			if ("Approved".equalsIgnoreCase(trimmed) || "Yes".equalsIgnoreCase(trimmed)
					|| "Proceed".equalsIgnoreCase(trimmed) || "OK".equalsIgnoreCase(trimmed)) {
				context.log("[KERNEL] User approved best trajectory via fast-approval.");
				return variants.stream().max((v1, v2) -> Double.compare(v1.getScore(), v2.getScore()))
						.map(v -> v.getId()).orElse(null);
			}

			if ("Reject".equalsIgnoreCase(trimmed) || "No".equalsIgnoreCase(trimmed)
			...
			if (lower.startsWith("select ") || lower.startsWith("approve variant ")) {
				...
				if (keepId.toLowerCase().startsWith("variant "))
					keepId = keepId.substring(8).trim();
				...
				variants.stream().filter(v -> v.getId().equals(finalKeepId)).findFirst().ifPresent(v -> {
					...
					if (genomeObj instanceof eu.kalafatic.evolution.controller.orchestration.selfdev.SemanticGenome) {
						String currentDim = (String) context.getOrchestrationState().getMetadata()
								.get("current_dimension");
						if (currentDim != null) {
							((eu.kalafatic.evolution.controller.orchestration.selfdev.SemanticGenome) genomeObj)
									.lockDimension(currentDim);
				...
				if (rejectedId.toLowerCase().startsWith("variant "))
					rejectedId = rejectedId.substring(8).trim();
				...
				variants.stream().filter(v -> v.getId().equals(finalRejectedId)).findFirst().ifPresent(v -> {
					...
					sessionContainer.getEvolutionMemoryGraph().recordRejection("MANUAL_SELECTION", finalRejectedId,
							"User rejected explicitly.");
					emitDarwinBranches(context, variants, null);
				});
				...
				if (genomeObj instanceof eu.kalafatic.evolution.controller.orchestration.selfdev.SemanticGenome) {
					...
					eu.kalafatic.evolution.controller.orchestration.selfdev.EvolutionDimension newDim = new eu.kalafatic.evolution.controller.orchestration.selfdev.EvolutionDimension(
							dimName.toUpperCase().replace(" ", "_"), dimName,
				...
				if (gmObj instanceof GoalModel) {
					...
					goalModel = new ObjectMapper().configure(DeserializationFeature.FAIL_ON_UNKNOWN_PROPERTIES, false)
				...
				context.log(
						"[KERNEL] User injected a new trajectory via prompt. Integrating as a first-class candidate.");
	...
	public IterationManager createVariantManager(TaskContext variantContext, AiService aiService) {
		try {
			return KernelFactory.create("Autonomous", variantContext, sessionContainer, aiService);
		} catch (Exception e) {
	...
	public void refineTargetReality(String goal, TaskContext context) throws Exception {
		...
		while (pass <= 3 && existingModel.getRealityCompleteness() < 0.85) {
			context.log("[KERNEL] Discovery Loop Pass " + pass + " (Completeness: "
					+ existingModel.getRealityCompleteness() + ")");
	...
	public String performMediatedExportConvergence(String request, TaskContext context) {
		try {
			...
			if (snapshotObj instanceof TargetSnapshot) {
				...
				try {
					snapshot = new ObjectMapper().configure(DeserializationFeature.FAIL_ON_UNKNOWN_PROPERTIES, false)
			...
			if (realityModelObj == null) {
				...
				try {
					GoalModel gm = GoalModel.extract(context.getOrchestrationState().getMetadata(), this, request,
							context);
					if (gm != null) {
					...
					context.log("[KERNEL] Warning: Could not extract GoalModel for fallback RealityModel: "
							+ e.getMessage());
				}
	...
	public boolean executeTasksWithRetries(List<Task> tasks, Runnable onStepComplete) throws Exception {
		...
		for (Task task : tasks) {
			...
			for (int retry = 1; retry <= EvolutionConstants.MAX_TASK_RETRIES; retry++) {
				...
				RuntimeEventBus bus = sessionContainer.getEventBus();
				bus.publish(new RuntimeEvent(RuntimeEventType.TASK_STARTED, context.getSessionId(), "Kernel",
						task.getId()));
				...
				if (evaluation.optBoolean("success", false)) {
					...
					bus.publish(new RuntimeEvent(RuntimeEventType.TASK_COMPLETED, context.getSessionId(), "Kernel",
							task.getId()));
